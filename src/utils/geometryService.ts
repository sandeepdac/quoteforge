...
export interface GeometryMilled {
  ...
  holeDepthsMm?: number[];
  /** Threads PROPOSED from the tap-drill ⌀, when nothing in the file named one. */
  threadCandidates?: Array<{
    callout: string; tapDrillMm: number; pitchMm: number;
    holeDiaMm: number; depthMm: number | null; source: string;
  }>;
