/**
 * Design-for-Manufacturing (DFM) analysis for MACHINED parts — turned and milled
 * alike, metals and plastics.
 *
 * It began as turning-only and was called only for turned parts, which meant a
 * milled part got no advisory findings AT ALL: no small-hole warning, no thin
 * wall, no deep hole, and — once threads were costed — no way to be told that a
 * thread had been assumed from a tap-drill diameter. Most of these rules never
 * depended on the part being round. The few that do (slenderness, bar capacity)
 * are guarded on a rod diameter, so they simply do not fire for a billet.
 *
...
export interface CncDfmInput {
  ...
  /**
   * Threads we believe this part carries, and where the belief came from.
   * A thread is the one operation with no geometric signature, so this is
   * always something for the quoter to check against the drawing.
   */
  threads?: Array<{ callout: string; count?: number }>;
  /** True when the threads above were PROPOSED from a tap-drill ⌀, not named. */
  threadsAreCandidates?: boolean;
  tolerances?: string;
...
export function analyzeCncDfm(input: CncDfmInput): DfmReport {
  ...
  // 8a) THREADS. Never visible in the faces, so this finding is the only place
  // a quoter is told what we assumed. It exists because the alternative was
  // silence: reading eleven Turncircuit drawings found a thread on every single
  // part and not one of them was priced.
  if (input.threads?.length) {
    const labels = input.threads
      .map((t) => `${Math.max(1, t.count ?? 1)}\u00d7 ${t.callout}`).join(', ');
    findings.push({
      id: 'threads',
      severity: 'info',
      title: input.threadsAreCandidates
        ? `Tapping assumed: ${labels} — confirm against the drawing`
        : `Tapping: ${labels}`,
      detail: input.threadsAreCandidates
        ? `Nothing in the CAD file names a thread, so these were proposed from the only fingerprint a thread leaves — a tapped hole is modelled at its TAP-DRILL diameter, and these holes sit exactly on one. Their tapping time IS in this quote. Check each against the drawing, and add any thread whose tap drill was not found.`
        : `Read from the model's own feature names and priced as a rigid tapping cycle. A CAD tree name records the last feature rather than an inventory, so confirm the count.`,
      rule: 'Threads are a drawing callout, never a measured face',
    });
  } else if (input.hasGeometry) {
    findings.push({
      id: 'threads',
      severity: 'info',
      title: 'No threads detected — check the drawing',
      detail: `Nothing in this solid names a thread and no hole sits on a tap-drill diameter, so no tapping time is in this quote. That is a real answer, not a silent one: a tapped hole is modelled as a plain cylinder, so if the drawing calls out a thread it has to be added by hand.`,
      rule: 'Threads are a drawing callout, never a measured face',
    });
  }
