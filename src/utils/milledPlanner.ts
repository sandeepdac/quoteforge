...
export function buildMilledPlan(inp: MilledPlanInput): MachiningPlan {
  ...
  const setups = Math.max(1, Math.round(inp.setups));
  const finishSetup = (phaseIdx: number) => Math.min(setups, 1 + (phaseIdx % Math.max(1, setups)));

  const ops: DraftOp[] = [];
  const add = (o: DraftOp) => {
    if (o.sec > 0.5) ops.push(o);
  };

  // --- Facing (once per setup — you skim each re-clamped face) --------------
  ...
  // --- Rest roughing (small cutter clears what the big one can't) ----------
  add({
    name: 'Rest roughing',
  ...
  // --- Detail finishing (small cutter on tight walls) ----------------------
  add({
  ...
  if ((inp.holeCount > 0 || inp.bossCount > 0) && cham) {
    ...
    if (chamSec > 0.5) {
      // reduce wall finishing by the same amount to conserve total time
      const wall = ops.find((o) => o.name === 'Wall finishing');
      if (wall) wall.sec = Math.max(0, wall.sec - chamSec);
      add({
