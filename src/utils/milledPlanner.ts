...
export interface MilledPlanInput {
  ...
  /** Seconds of TAPPING, and the threads it covers. */
  tapSec?: number;
...
export function buildMilledPlan(inp: MilledPlanInput): MachiningPlan {
  ...
  // --- Tapping: its own tool, and a cycle the traveller has to show --------
  if ((inp.tapSec ?? 0) > 0) {
    const th = inp.threads ?? [];
    addSub({
      name: 'Tapping',
  ...
  for (let s = 1; s <= setups; s++) {
    ...
    planSetups.push({
      ...
      // shows a separate "Setup labour" line for preparing the machine.
      name: `Op ${s}`,
      operations,
      seconds,
      cost,
