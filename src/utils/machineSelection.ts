...
export function buildRoute(
  ...
  const first: RouteOp = {
    ...
    // OBSERVED figure, kept for comparison only — the PRICE now uses the derived
    // model in setupModel.ts. Four of these seven numbers came from a single job
    // each, so as a price they were a lookup of Lance's answers rather than an
    // estimate, and could not speak at all for a machine he had not quoted.
