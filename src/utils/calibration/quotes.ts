 ...
 * cleaning, the lot. The right figure for "what does the whole job cost per
 * part", and the WRONG one to score a cycle-time model against.
 */
 ...
 * against, because it is the only thing that model is trying to predict.
 *
 ...
 * 15 min of inspection; the collet block 1.9 min against 3.57 all-in. Scored on
 * the all-in figure the cycle model read x0.60 on that part when it was really
 * x0.91 — so a third of the error we have been chasing was work the model never
 * claimed to do.
 *
 * Both are kept on purpose: price needs every minute, cycle accuracy needs only
 * the spindle's.
 */
