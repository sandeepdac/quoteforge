...
import { extractTurnedProfile, arrayBufferToBase64, GeometryResult, GeometryMilled } from './geometryService';
import type { ThreadSpec } from './drilling';

/**
 * Threads the geometry service could find, as costable specs.
 *
 * Two sources, in order of authority. A callout NAMED in the model is the
 * stronger one — the CAD system wrote it because a designer asked for it. A
 * candidate proposed from the tap-drill ⌀ is weaker but it is what we usually
 * have: none of the seven Turncircuit files carries a single feature name, and
 * every one of those parts has a thread on its drawing.
 *
 * Depth falls back to the hole's own depth: you tap as far as the hole goes
 * unless the drawing says otherwise, and it usually says a little less. Where
 * even that is unknown, a thread is assumed to run about twice its diameter —
 * the usual rule of engagement — because letting it default to zero would price
 * the thread at nothing, which is the bug this whole path exists to fix.
 */
const THREAD_DEPTH_UNKNOWN = (tapDrillMm: number) => 2 * tapDrillMm;
function threadsFromGeometry(mm: GeometryMilled): ThreadSpec[] {
  const named = (mm.threadCallouts ?? [])
    .filter((c) => c.matchedHoleCount > 0)
    .map((c) => ({
      callout: c.callout,
      // A named callout carries no pitch, so use the coarse pitch the tap-drill
      // table implies for that size.
      pitchMm: Math.max(0.2, c.majorDiaMm - c.tapDrillMm),
      tapDrillMm: c.tapDrillMm,
      depthMm: THREAD_DEPTH_UNKNOWN(c.tapDrillMm),
      count: c.matchedHoleCount,
    }));
  if (named.length) return named;
  return (mm.threadCandidates ?? []).map((c) => ({
    callout: c.callout,
    pitchMm: c.pitchMm,
    tapDrillMm: c.tapDrillMm,
    depthMm: c.depthMm ?? THREAD_DEPTH_UNKNOWN(c.tapDrillMm),
    count: 1,
  }));
}
...
async function analyzeSolid(
  ...
  if (meas && mesh) {
    ...
    // Threads, for BOTH the DFM finding and the turned profile. A thread has no
    // face to measure, so whichever route the part takes it needs telling.
    const svcMilled = useSvc ? svc!.milled : undefined;
    const dfmThreads = svcMilled ? threadsFromGeometry(svcMilled) : [];
    const dfmThreadsAreCandidates = !!svcMilled
      && !(svcMilled.threadCallouts ?? []).some((c) => c.matchedHoleCount > 0)
    ...
    if (!isTurned) {
      if (useSvc && svc!.milled) {
        const mm = svc!.milled;
        // THREADS. The solid holds only the tap drill, so these are either named
        // in the file or proposed from that ⌀ — never inferred from a face. Both
        // sources are candidates a quoter confirms against the drawing; what
        // they replace is a threaded part quoting as though it had no threads.
        const threadSpecs = threadsFromGeometry(mm);
    ...
    // The deepest hole RELATIVE to its diameter — the one a deep-hole warning is
    // actually about. A milled part has no single "bore", but it can certainly
    // have a hole that needs pecking or gun-drilling: the Cold Stage Block's
    // ⌀1.0 runs 25 mm, an L/D of 25.
    const deepestHole = (() => {
      const dias = milledProfile?.holeDiametersMm ?? [];
      const depths = milledProfile?.holeDepthsMm ?? [];
      let best: { diameterMm: number; depthMm: number } | undefined;
      dias.forEach((d, i) => {
        const z = depths[i];
        if (!d || !z) return;
        if (!best || z / d > best.depthMm / best.diameterMm) best = { diameterMm: d, depthMm: z };
      });
      return best;
    })();

    // Advisory DFM. Runs for BOTH routes — the rod-specific rules are guarded on
    // a diameter a billet does not have, so they skip themselves.
    const dfm = isTurned
      ? analyzeCncDfm({
          partClass: 'turned',
          thicknessMm,
          ...
          threads: dfmThreads,
          threadsAreCandidates: dfmThreadsAreCandidates,
          hasGeometry: !!geo,
        })
      : analyzeCncDfm({
          partClass: 'milled',
          thicknessMm,
          boundingBoxMm: { lengthMm: meas.lengthMm, widthMm: meas.widthMm, heightMm: meas.heightMm },
          // No rod: slenderness and bar capacity self-skip on these.
          diameterMm: 0,
          axisLengthMm: 0,
          holeDetails,
          buyToFlyRatio,
          setups,
          crossFeatures,
          boreDiaMm: deepestHole?.diameterMm,
          boreDepthMm: deepestHole?.depthMm,
          maxDrillDiaMm: DEFAULT_CNC_SETTINGS.maxDrillDiaMm,
          tolerances: 'Standard ISO 2768-m (±0.2mm)',
          threads: dfmThreads,
          threadsAreCandidates: dfmThreadsAreCandidates,
