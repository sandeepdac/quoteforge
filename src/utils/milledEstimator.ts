...
export interface MilledProfile {
  ...
  /**
   * TAPPED threads, as callouts. A thread leaves no face to measure — the solid
   * holds only the tap drill — so these are read from the model's names or
   * proposed from the tap-drill ⌀, never inferred from geometry. Every part in
   * the calibration set has at least one and none of them was priced.
   */
  threads?: ThreadSpec[];
...
export function calculateMilledCosts(
  ...
  const derivedSetup = routeOps?.length ? deriveRouteSetup(routeOps, {
    toolCount,
    featureCount: (p.holeDiametersMm ?? []).length
