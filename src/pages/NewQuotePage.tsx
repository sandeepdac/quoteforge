      ...
      <div className="mt-4 transition-all duration-300">
        ...
        {currentStep === 2 && (
          <StepExtract
            ...
            onThreadsChange={(threads) => setCadAnalysis((prev) => {
              // Threads belong to the PART, so they go on whichever profile is
              // costing it. Without this the editor would look like it worked
              // and change nothing — the failure mode this whole feature exists
              // to prevent, just moved one screen along.
              if (!prev) return prev;
              const same = JSON.stringify(
                prev.turningProfile?.threads ?? prev.milledProfile?.threads ?? []
              ) === JSON.stringify(threads);
              if (same) return prev;
              return {
                ...prev,
                turningProfile: prev.turningProfile
                  ? { ...prev.turningProfile, threads } : prev.turningProfile,
                milledProfile: prev.milledProfile
