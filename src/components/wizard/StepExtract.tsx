...
import { THREAD_CATALOG, threadFromCallout } from '../../utils/drilling';
import type { ThreadSpec } from '../../utils/drilling';
...
interface StepExtractProps {
  ...
  /**
   * Threads the quoter confirmed or added. A thread has no face to measure, and
   * an EXTERNAL one cannot even be guessed — its major diameter is the same
   * round number a plain shaft uses — so this is the only way some of them can
   * ever reach a price.
   */
...
export default function StepExtract({ cadAnalysis, materialId, onContinue, onBack, onSnapshot, savedThumbnail, onThreadsChange }: StepExtractProps) {
  ...
  const detectedThreads = (cadAnalysis?.turningProfile?.threads
    ?? cadAnalysis?.milledProfile?.threads ?? []) as ThreadSpec[];
    ...
    <div className="max-w-6xl mx-auto space-y-6 animate-in slide-in-from-bottom-4 duration-500 pb-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-6 space-y-6">
          ...
          {/* THREADS — the one operation with no geometric signature.
              Detection handles tapped holes (a tap drill is an odd size nothing
              else lands on) but can never handle external threads, so this is
              where a quoter reconciles the model with the drawing. */}
          {cadAnalysis && (
            <div className="bg-card border border-border p-5 rounded-xl space-y-3">
              <h3 className="text-xs font-bold uppercase tracking-wider text-foreground border-b border-border pb-2.5 flex items-center justify-between">
                ...
                <span className={cn(
                  ...
                  {threads.length ? `${threads.reduce((a, t) => a + (t.count ?? 1), 0)} TAPPED` : 'NONE FOUND'}
                </span>
              </h3>
              <p className="text-[10px] text-muted-foreground leading-relaxed">
                A tapped hole is stored in CAD as a plain cylinder at its tap-drill size, so a
                thread can only be <strong className="text-foreground">proposed</strong> from that
                size — and an external thread cannot be seen at all, because its diameter is the
                same number a plain shaft uses. Check these against the drawing.
              </p>
              {threads.map((t, i) => (
                <div key={i} className="flex items-center gap-2">
                  <select
                    className="flex-1 bg-background border border-border rounded-md px-2 py-1.5 text-xs font-semibold"
                    value={t.callout}
                    onChange={(e) => {
                      const next = threadFromCallout(e.target.value, t.depthMm, t.count ?? 1);
                      if (next) setThreads(threads.map((x, j) => (j === i ? next : x)));
                    }}
                  >
                    {Object.keys(THREAD_CATALOG).map((c) => <option key={c} value={c}>{c}</option>)}
                  </select>
                  <label className="text-[10px] text-muted-foreground">x</label>
                  <input
                    type="number" min={1} aria-label="count"
                    className="w-14 bg-background border border-border rounded-md px-2 py-1.5 text-xs font-semibold text-center"
                    value={t.count ?? 1}
                    onChange={(e) => setThreads(threads.map((x, j) =>
                      j === i ? { ...x, count: Math.max(1, Number(e.target.value)) } : x))}
                  />
                  <label className="text-[10px] text-muted-foreground">deep</label>
                  <input
                    type="number" step="0.1" min={0} aria-label="depth mm"
                    className="w-20 bg-background border border-border rounded-md px-2 py-1.5 text-xs font-semibold text-center"
                    value={Math.round(t.depthMm * 10) / 10}
                    onChange={(e) => setThreads(threads.map((x, j) =>
                      j === i ? { ...x, depthMm: Math.max(0.1, Number(e.target.value)) } : x))}
                  />
                  <button
                    onClick={() => setThreads(threads.filter((_, j) => j !== i))}
                    className="px-2 py-1.5 rounded-md border border-border text-[10px] hover:bg-accent"
                  >Remove</button>
                </div>
              ))}
              <button
                onClick={() => {
                  const t = threadFromCallout('M3', 0, 1);
                  if (t) setThreads([...threads, t]);
                }}
          ...
          {isMachined && cadAnalysis?.machineRecommendation && (() => {
            ...
            return (
              <div className="bg-card border border-border p-5 rounded-xl space-y-4">
                ...
                {/* THE ROUTE. A part that runs on two machines was described by
                    one machine's name, because only the primary was ever shown —
                    while the second op's setup was quietly in the price. Naming
                    both is what lets an estimator check the answer against a
                    router, which is the form the shop actually works from. */}
                {(mr.machineRoute?.ops.length ?? 0) > 1 && (
                  <div className="space-y-1.5">
                    <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">
                      Route — {mr.machineRoute!.machines.length} machine{mr.machineRoute!.machines.length === 1 ? '' : 's'}
                    </p>
                    {mr.machineRoute!.ops.map((op, i) => (
                      <div key={i} className="flex items-start gap-2 rounded-md border border-border bg-accent/30 p-2 text-[11px]">
                        <span className="shrink-0 rounded bg-primary/10 px-1.5 py-0.5 font-bold text-primary text-[10px]">
                          Op {i + 1}
                        </span>
                        <div className="min-w-0">
                          <span className="font-semibold text-foreground">{op.machineName}</span>
                          <span className="text-muted-foreground"> · {op.setups} holding{op.setups === 1 ? '' : 's'}</span>
                          <p className="text-muted-foreground/80 leading-tight">{op.reason}</p>
                        </div>
                      </div>
                    ))}
                    <p className="text-[10px] text-muted-foreground/70 leading-relaxed">
                      Each machine is dialled in separately, and the setup line on the next step shows
                      what each one costs.
                    </p>
                  </div>
                )}
                ...
                <p className="text-[10px] text-muted-foreground/70 leading-relaxed">
                  The primary machine's charge-out ({symbol}{MACHINE_CATALOG[mr.recommended]?.hourlyRate ?? '—'}/hr) is applied to
                  ALL cycle time{(mr.machineRoute?.machines.length ?? 1) > 1 ? `, including the work that runs on the ${mr.machineRoute!.ops[mr.machineRoute!.ops.length - 1].machineName}` : ''} —
                  we know each op's machine but not how the cutting minutes split between them, and
                  splitting them by holdings scored worse than leaving them alone. Setup IS charged per
                  machine. Rates are defaults for a UK precision shop — set your own before sending a quote.
                </p>
              </div>
            );
          })()}
