...
import { SAMPLE_CAD_PDF_METADATA } from '../../utils/sampleCadFiles';
...
export default function CadPdfViewer({ pdfFileName = 'Drawing.pdf', pdfData = SAMPLE_CAD_PDF_METADATA, className = '' }: CadPdfViewerProps) {
  const [zoom, setZoom] = useState(1);
  const [showOverlays, setShowOverlays] = useState(true);
  const [isBlueprintTheme, setIsBlueprintTheme] = useState(true);
  const [activeTab, setActiveTab] = useState<'drawing' | 'titleblock' | 'notes'>('drawing');
        ...
        >
          ...
          <div className="absolute bottom-2 right-2 border border-sky-400/60 bg-sky-950/90 text-slate-200 text-[9px] font-mono p-2 rounded w-52 shadow-lg">
            ...
            <div className="grid grid-cols-2 gap-x-2 pt-1 text-[8px] text-slate-300">
              ...
              <div>MAT: <strong className="text-white">Mild Steel 3mm</strong></div>
