    ...
    <div className="bg-card border border-border rounded-xl overflow-hidden">
      <div className="px-4 py-3 bg-muted/30 border-b border-border flex items-center justify-between gap-3 flex-wrap">
        ...
        <div className="text-[11px] text-muted-foreground">
          <span className="font-semibold text-foreground">{secs(plan.totalSeconds)}</span> cutting per part ·{' '}
          <span className="font-semibold text-foreground">{money(plan.totalCost)}</span>
          {setupTimeMin ? (
            <> · <span className="font-semibold text-foreground">{Math.round(setupTimeMin)} min</span> to set up, once per batch</>
          ) : null}
        </div>
      </div>

      {/* Fixturings ("Op N") and the operations inside them. The seconds on each
          row are CUTTING time for one part — setup is the separate line above,
      ...
      <div className="px-4 pb-3 pt-1 flex items-start gap-2">
        ...
        <p className="text-[10px] text-muted-foreground leading-relaxed">
          Operations and tools are <strong className="text-foreground">inferred from the geometry</strong> and your shop tool library, not
          read from a posted toolpath. The setup count is measured; operations are grouped into setups by phase (roughing &amp; drilling,
          then finishing &amp; edge-break). Treat the per-operation times as indicative and the <strong className="text-foreground">total</strong> as the estimate.
        </p>
      </div>
    </div>
  );
