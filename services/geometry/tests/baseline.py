...
TRACKED = [
    ...
    # Threads are now COSTED, so a change in how many we propose changes a price.
    "threadCandidateCount",
]
...
def measure_one(path: str) -> dict:
    ...
    # The turned profile and the thread candidates are produced by the EXTRACTOR,
    # not by the milling analysis on its own, so read them from a full extract.
    full = extract(path) or {}
    profile = full.get("profile") or {}
    m["crossFeatureCount"] = len(profile.get("crossFeatureList") or [])
    m["threadCandidateCount"] = len((full.get("milled") or {}).get("threadCandidates") or [])
