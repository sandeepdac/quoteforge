...
def labelled_mesh_b64(payload: Base64Payload):
    ...
    try:
        ...
        # Threads have no face to measure, so this endpoint has to ask about them
        # too — it builds its own analysis and would otherwise show a green
        # "fully accounted" badge over a part with untapped holes. Both the
        # NAMED callouts and the ones proposed from the tap-drill ⌀, through the
        # same helper the extract endpoint uses so the two cannot drift apart.
        callouts = match_threads_to_holes(
            find_thread_callouts(tmp.name), milled.get("holeDiametersMm") or []
        )
        candidates = find_thread_candidates(
            milled.get("holeDiametersMm") or [], milled.get("holeDepthsMm") or []
        )
        milled["openQuestions"] = thread_open_questions(callouts, candidates)
