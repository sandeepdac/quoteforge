...
def extract(path: str) -> dict:
    ...
    while exp.More():
        ...
        if stype == GeomAbs_Cylinder:
            ...
            if coaxial:
                ...
                # How far around its axis this face wraps. A drilled hole closes
                # the full turn (often as two halves); a corner FILLET is the same
                # kind of cylindrical face but only sweeps ~90°. Without this the
                # two are indistinguishable, and every rounded corner on a part
                # was being billed as a driven-tool operation.
                _u0, _u1, _, _ = BRepTools.UVBounds_s(face)
                entry["_span"] = abs(_u1 - _u0)
                entry["_axis"] = cdir
                entry["_loc"] = cloc
    ...
    # Which of these are real OPERATIONS, and which are corner rounds?
    #
    # A rounded corner is a cylindrical face just like a drilled hole, and until
    # this test every one of them was billed as a driven-tool operation with its
    # own spindle orient and tool change. Lance's C clamp carries R1.0 in six
    # places and R2.0 in four; that is ten phantom operations on a part with
    # seven real ones, and it is the reason that part quoted 36% high.
    #
    # The test turns on which SIDE the material is, and the drawings settle it.
    #
    # An INTERNAL (concave) cylinder has material all around it. That is a hole,
    # a bore, a counterbore or a milled radius — something a tool had to go into.
    # It is never a rounded outside corner, so it is always real work. Judging
    # these on size was tried first and threw away the C clamp's ⌀10 counterbores
    # and the hollow arm's six ⌀0.7 holes, which the drawings call out explicitly.
    #
    # An EXTERNAL (convex) cylinder is the ambiguous one: it is either a genuine
    # profile the cutter walks around, or the round left where two faces meet. A
    # real one either closes the full turn or is far too big to be a cutter
    # corner. On the C clamp that keeps the ⌀50.8 throat and drops R1.0 in six
    # places and R2.0 in four; on the hollow arm it drops R1.15 in four.
    #
    # The size gate is relative to the PART. An absolute floor dismisses every
    # feature on an 8 mm part, which is the trap the milling analyser fell into
    # on this very bulkhead.
    FULL_TURN = 2.0 * math.pi
    smallest = min(bbox.values())
    outer_corner_dia_max = min(max(12.0, 0.12 * max(bbox.values())),
                               0.5 * max(smallest, 1.0))

    def _is_real_cross(g: dict) -> bool:
        if g["isBore"]:
            return True
        if g["span"] >= 0.85 * FULL_TURN:
            return True
        return 2.0 * g["radiusMm"] > outer_corner_dia_max
    ...
    # When nothing was NAMED, propose threads from the holes themselves. A tap
    # drill is the only fingerprint a thread leaves in a solid, and on the parts
    # we hold drawings for it finds every one of them and invents none. These
    # files carry no feature names at all — the name route found nothing on any
    ...
    # the shared helper so this endpoint and the mesh endpoint cannot disagree.
    open_questions = thread_open_questions(thread_callouts, milled["threadCandidates"])
